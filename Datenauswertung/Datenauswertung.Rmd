...
* corrMAtrix aufstellen, um mögliche Zusammenhänge von Variablen zu erkennen
* Erkenntnis bei uns vielleicht, dass bestimmte hier untersuchte Variablen **nicht** mit dem Switch-off in Verbindung stehen. Für Ausblick, dass man weitere Variablen herausfinden muss.
...
```
Es gibt einen signifikanten Unterschied zwischen der Arbeitszufriedenheit und dem Switch off (p < .001).
Es gibt einen signifikanten Unterschied zwischen der Arbeitszeit und dem Switch off F(2) = 11.7, p < .001
...
Es gibt keinen signifikanten unterschied zwischen der Arbeitszeit und dem Switch off (p > .05). Anders ist es bei der Arbeitintensität und dem Switch off, hier gibt es einen signifikanten Unterschied (p < .001). 
Betrachtet man Arbeitszeit und -intensität in Bezug auf den Switch off, gibt es wiederum keinen signifikanten Unterschied (p > .05).
